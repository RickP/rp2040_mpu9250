...
bool i2c_read(uint8_t addr, uint8_t reg, uint8_t count, uint8_t * dst) {
    // printf("Read - reg: 0x%X - count: %i\n", reg, count);
    i2c_write_blocking(i2c, addr, &reg, 1, true);
    i2c_read_blocking(i2c, addr, dst, count, false);
    return false;
}

bool i2c_write(uint8_t addr, uint8_t reg, uint8_t count, uint8_t * src) {
    uint8_t buffer[count+1];
    buffer[0] = reg;
    // printf("Write - reg: 0x%X - src: %i\n", reg, src);
    for (int i=0; i < count; i++) {
        buffer[i+1] = src[i];
    }
    i2c_write_blocking(i2c, addr, buffer, count+1, false);

    return false;
}

bool mpu9250_init(i2c_inst_t *i2c_instance, uint8_t sda_pin, uint8_t scl_pin) {
    i2c = i2c_instance;
...
static int get_st_6500_biases(long *gyro, long *accel, unsigned char hw_test, int debug)
{
    ...
    sleep_ms(test.wait_ms);  //wait 200ms for sensors to stabilize
    ...
    while (s < test.packet_thresh) {
        sleep_ms(test.sample_wait_ms); //wait 10ms to fill FIFO
        if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, data))
            return -1;
        fifo_count = (data[0] << 8) | data[1];
        packet_count = fifo_count / MAX_PACKET_LENGTH;
